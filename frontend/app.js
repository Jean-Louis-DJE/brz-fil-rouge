...
function showNotification(message, isError = false) {
    ...
    // Si c'est une erreur, mettre le fond en rouge
    if (isError) {
        toast.style.backgroundColor = '#f44336'; // Rouge
    } else {
        toast.style.backgroundColor = '#4caf50'; // Vert par défaut
    }
...
// A. DÉCLARATION GLOBALE DE LOADPROFILE (À METTRE AVEC LES AUTRES FONCTIONS)
async function loadProfile() {
    try {
        // ... (votre code loadProfile est ici)
        ...
        // Mettre à jour les images (Menu + Page Profil)
        if (user.avatar) {
        ...
        document.getElementById('menu-username').textContent = `${user.prenom} ${user.nom}`;
...
// B. DÉCLARATION GLOBALE DE SAVEPROFILE (À METTRE AVEC LES AUTRES FONCTIONS)
async function saveProfile() {
    ...
    formData.append('habitudes', document.getElementById('input-habitudes').value);
    
    // Ajouter l'image seulement si l'utilisateur en a choisi une nouvelle
    ...
    try {
        ...
        if (result.success) {
            ...
            loadProfile(); // Recharge pour voir les changements partout
        } else {
            showNotification("Erreur de sauvegarde: " + result.error, true); 
        }
    } catch (err) {
        console.error(err);
        showNotification("Erreur technique de connexion au serveur.", true); 
    }
}


// C. BLOC D'INITIALISATION FINAL (à la fin du fichier)
// Le code est plus court et appelle les fonctions maintenant globales.
(async () => {
    const ctx = document.getElementById('historyChart').getContext('2d');
    
    // --- Définir les dates par défaut (PAS DE CHANGEMENT ICI) ---
    ...
    // Initialisation du graphique principal (PAS DE CHANGEMENT ICI)
    chart = new Chart(ctx, {
        type: 'line',
    ...
    // 2. CHARGEMENT INITIAL DES KPI
    await updateDashboard();
    
    // 3. CHARGEMENT INITIAL DES GRAPHIQUES
    await updateHomeChart();
    await drawDailyCostChart();
    
    // 4. MISE EN PLACE DES ÉCOUTEURS POUR LES BOUTONS JOUR/MOIS
    setupRangeListeners();

    // 5. BOUCLE DE SIMULATION (PAS DE CHANGEMENT ICI)
    setInterval(async () => {
